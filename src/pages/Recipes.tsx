...
import { openDb, seedIfEmpty, getAllRecipes, searchRecipesByName, addRecipe, updateRecipe, type RecipeSort } from '../db'
...
export default function Recipes(){
  ...
  const [form, setForm] = useState({ name: '', type: 'Dinner' as any })
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 pb-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {recipes.map(r => (
          <RecipeCard key={r.id} name={r.name} subtitle={r.notes ?? undefined} />
        ))}
      </div>
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl p-4 border border-gray-200 shadow" onClick={e=>e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div>
                ...
                <select className="w-full border border-gray-200 rounded-xl px-3 py-2" value={form.type} onChange={e=>setForm({...form, type: e.target.value})}>
            ...
            <div className="flex gap-2 justify-end mt-4">
              ...
              <button className="px-4 py-2 rounded-xl bg-blue-600 text-white font-semibold"
                onClick={()=>{
                  ...
                  addRecipe(db, { name: form.name.trim(), type: form.type as any })
                  setAddOpen(false)
